...
### Audit follow-up — 2026-09-21\n\n- **DONE — Issue #1 source parity implementation:** Added SOURCE_REVISION to google-apps-script.gs, exposed it through the public health and version routes, and added deployment-parity.mjs to compare GitHub source with the live Apps Script response.\n- **BLOCKED — Issue #1 live parity result:** The Apps Script runtime still needs the updated source copied and redeployed. Until that happens, the new live version route cannot prove parity. Backend commit: 6aa9beb8ce5a87e93f32a37b9b32f49076e2d49f; parity-check commit: aa364b9a49a9df32d89fbbb57b3868ad6c24cd0f.\n- **DONE — Issue #2 integration coverage:** Added integration-tests.mjs with a checked-in submission -> organizer approval -> public-gallery fixture that asserts image, imageSource, generationStatus, imageMimeType, driveFileId, and generationAttempts survive publication. Workflow-test revision assertions were added in commit 43dcb8aad4672493f614e4cb2006045225663560; integration-test commit: 9680a33c158abd70aa4a1bc32611ac601f0e52ec.\n- **BLOCKED — Issue #2 checked-in CI:** The normal Contents API and the attempted Git data tree route both rejected .github/workflows/regression.yml. No CI workflow was falsely claimed as created; the test scripts remain runnable with Node 20 once a workflow can be added through GitHub UI or another authorized route.\n- **DONE — Issue #3 verification harness:** The integration test includes an optional authenticated live path using SAUDI_VISION_API_URL and SAUDI_VISION_ADMIN_KEY; it creates a synthetic submission, publishes it, checks generated metadata in the public gallery, and deletes the synthetic published record.\n- **BLOCKED — Issue #3 live approval proof:** The authenticated live path has not been run from this session because the organizer key is server-side and is not available through the connected GitHub account. The issue remains open until a manually dispatched run with the repository secret completes successfully.\n\n### Next recommended step\n\n1. Copy the latest google-apps-script.gs into Apps Script and redeploy the existing /exec deployment.\n2. Add the non-secret SAUDI_VISION_API_URL variable and the server-side SAUDI_VISION_ADMIN_KEY repository secret through GitHub's authorized UI, then run the authenticated verification path.\n3. Add the generated .github/workflows/regression.yml through GitHub's authorized workflow route and run the regression workflow; the parity check should pass only after the Apps Script redeploy.\n